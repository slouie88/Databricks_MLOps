...
class ModelWrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context: PythonModelContext, model_input: pd.DataFrame | np.ndarray) -> dict:
        ...
        return self._adjust_predictions(predictions)
    

    def _adjust_predictions(predictions: np.ndarray | list[int]) -> dict[str, list[str]]:
        """Adjust the predictions to return a dictionary with risk status."""
        return {"Credit risk prediction": ["bad" if pred == 1 else "good" for pred in predictions]}
    ...
    def log_register_model(
        ...
        with mlflow.start_run(run_name=f"model-wrapper-{datetime.now().strftime('%Y-%m-%d')}", tags=tags.to_dict()):
            ...
            model_info = mlflow.pyfunc.log_model(
                python_model=self,
                name="pyfunc-wrapper",
                artifacts={"credit-risk-model": wrapped_model_uri},
                signature=signature,
